class PollsController < ApplicationController
  before_action :set_poll, only: [:edit, :update, :show, :destroy, :close]
  before_action :require_login, only: [:create, :update, :new, :destroy, :close, :edit]
  before_action :require_non_admin, only: [:create]

  def index
    @polls = Poll.where(status: 'open').order(created_at: :desc)
  end

  def new
    @poll = current_user.polls.new
    3.times { @poll.options.build }
  end

  def create
    @poll = current_user.polls.new(poll_params.merge(user_email: current_user.email))
    if @poll.save
      redirect_to @poll, notice: 'Enquete criada com sucesso'
    else
      render :new, status: :unprocessable_entity
    end
  end

  def show
    @vote = Vote.new
  end

  def edit
    # Verificação de propriedade
    unless @poll.user == current_user
      redirect_to polls_path, alert: 'Você não tem permissão para editar esta enquete.'
    end
    @poll.options.build if @poll.options.empty?
  end

  def update
    # Verificação de propriedade
    unless @poll.user == current_user
      redirect_to polls_path, alert: 'Você não tem permissão para editar esta enquete.'
      return
    end

    if @poll.update(poll_params)
      redirect_to @poll, notice: 'Enquete atualizada com sucesso'
    else
      render :edit, status: :unprocessable_entity
    end
  end

  def destroy
    # Verificação de propriedade
    unless @poll.user == current_user
      redirect_to polls_path, alert: 'Você não tem permissão para excluir esta enquete.'
      return
    end
    
    @poll.destroy
    redirect_to polls_path, notice: 'Enquete excluída com sucesso'
  end

  def close
    if current_user == @poll.user || current_user.user_type == 'admin'
      @poll.closed!
      redirect_to @poll, notice: 'Enquete fechada com sucesso'
    else
      redirect_to @poll, alert: 'Apenas o dono pode fechar a enquete'
    end
  end

  def my_polls
    @polls = current_user.polls.order(created_at: :desc)
  end

  def voted_polls
    @voted_polls = current_user.voted_polls
  end

  def closed_polls
  @polls = Poll.where("status = ?", 'closed').order(created_at: :desc)
  end

  private
  def set_poll
    @poll = Poll.find_by(id: params[:id])
  end

  def poll_params
    params.require(:poll).permit(
      :question, :poll_type, :user_email,
      options_attributes: [:id, :content, :_destroy]
    )
  end
end
